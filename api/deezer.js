...
class Deezer {
	...
	async getTrackStream(track, flac = false) {
		if (track?.constructor !== Object) throw new TypeError("`track` must be an object.");
		...
		return new Promise((resolve, reject) => {
			request(url, (res) => {
				...
				const decryptStream = new Transform({
					transform(chunk, encoding, callback) {
						...
						while (buffer.length >= 2048) {
							let block = Buffer.alloc(2048);
							buffer.copy(block, 0, 0, 2048);
							buffer = buffer.slice(2048);

							if (chunkIndex % 3 === 0) {
								let dec = blowfish.cbc(blowfishKey, Buffer.from([0, 1, 2, 3, 4, 5, 6, 7]), block, true).toString("binary");
								block = Buffer.from(dec, "binary");
							}
							this.push(block);
							chunkIndex++;
						}
					...
					flush(callback) {
						if (buffer.length > 0) {
							let block = buffer;
							if (chunkIndex % 3 === 0 && block.length === 2048) {
								let dec = blowfish.cbc(blowfishKey, Buffer.from([0, 1, 2, 3, 4, 5, 6, 7]), block, true).toString("binary");
								block = Buffer.from(dec, "binary");
							}
							this.push(block);
				...
				const totalSize = parseInt(res.headers['content-length'] || track[`FILESIZE_${format}`]);
				resolve({ stream: res.pipe(decryptStream), size: totalSize });
			}).on('error', reject).end();
		});
	}
}
